...
const addEntryToSTS = async (sts_id, manager_id, entry_time, vehicle_id) => {
...
const getOnlyEntriesOfSTS = async (sts_id) => {
...
const addDumpEntryToSTS = async (sts_id, manager_id, entry_time, volume) => {
...
const getVehiclesOfSTS = async (sts_id) => {
  const query = `SELECT * FROM public."Vehicle" WHERE sts_id = $1 and disabled = false`;
  const result = await pool.query(query, [sts_id]);
  return result.rows;
};

const getSTSIDfromManagerID = async (manager_id) => {
